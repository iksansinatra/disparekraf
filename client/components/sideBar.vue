...
<!-- <pre>{{$store.state.list_menu}}</pre> -->
      ...
      <v-list>
      <v-img lazy-src="https://picsum.photos/id/11/10/6" style="margin-top:-3%" max-height="100%" max-width="100%" src="/img/ekokraf.png"></v-img>
         ...
         <v-list-item to="/" router exact>
          ...
          <v-list-item-content style="text-align:left; margin-left:-20px" >
            ...
            <v-list-group :value="false" no-action :key="i" v-if="item.type == 1 && checkLength(item.subItem)">
              ...
              <template v-for="(item2, i2) in item.subItem">
                ...
                <v-list-item link :key="i2" :to="item2.route" v-if="item2.type == 0">
                  <v-list-item-title class="h_menu1" style="; margin-left:-20px">{{item2.title}}</v-list-item-title>
                </v-list-item>
                <!-- ========================== SUB MENU ========================== -->
                ...
                <!-- ========================== SUB- SUB MENU ========================== -->
                  <v-list-group :value="false" no-action sub-group ripple :key="i2" v-if="item2.type == 1 && checkLength(item2.subItem)">
                    <template v-slot:activator>
                      <v-list-item-content style="text-align:left; margin-left:-10px">
                        <v-list-item-title class="h_menu1">{{item2.title}}</v-list-item-title>
                      </v-list-item-content>
                    </template>

                    <v-list-item link v-for="(item3, i3) in item2.subItem" :key="i3" :to="item3.route" style="margin-left:-20px">
        ...
        <v-list-item router exact @click="logout()">
          <v-list-item-action>
            <v-icon class="h_iconMenu">mdi-account-key</v-icon>
          </v-list-item-action>
