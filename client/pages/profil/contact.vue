    ...
    <v-container>
      <v-card class="mx-auto" outlined>
        <v-row class="subBar" style="padding:10px" no-gutters>
          <v-col cols="12" md="4">
            <span class="h_judul">Kontak</span>
            <br />
            <span class="h_Sub_judul">Profil</span>
          </v-col>
              ...
              <v-col cols="1">
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn tile height="37.5px" elevation="2" color="primary" v-bind="attrs" v-on="on" @click="mdl_add = true" :disabled="falseButton">
          ...
          <template v-slot:default>
            <thead style="background:#5289E7">
              <tr class="h_table_head">
                ...
                <th class="text-center" style="width:25%">HP</th>
        ...
        <v-dialog v-model="mdl_add" persistent max-width="600px">
          ...
          <v-card>
            ...
            <v-card-text>
                ...
                <div class="divInput">
                  <small>Alamat</small>
                  <v-textarea v-model="form.alamat" outlined name="input-7-4"></v-textarea>
                </div>
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
  ...
  export default {
    ...
    methods: {
      ...
      getView : function(){
        ...
        fetch(this.$store.state.url.URL_CONTACT + "view", {
            method: "POST",
            headers: {
      ...
      addData : function(number) {
        ...
        fetch(this.$store.state.url.URL_CONTACT + "addData", {
            method: "POST",
            headers: {
              "content-type": "application/json",
              authorization: "kikensbatara " + localStorage.token
      ...
      editData : function(){
        fetch(this.$store.state.url.URL_CONTACT + "editData", {
            method: "POST",
            headers: {
              "content-type": "application/json",
              authorization: "kikensbatara " + localStorage.token
      ...
      removeData : async function(data){
        ...
        fetch(this.$store.state.url.URL_CONTACT + "removeData", {
            method: "POST",
            headers: {
              "content-type": "application/json",
              authorization: "kikensbatara " + localStorage.token
